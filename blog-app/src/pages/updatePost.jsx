...
const UpdatePost = () => {
  ...
  const handleUpdate = async (e) => {
    try {
      ...
      setIsUpdate(true);
      if (!isLogin) {
        setfinalResult({ message: "User must be logged in" });
      } else {
        ...
        const res = await fetch(`http://localhost:3000/api/blog/${Post._id}`, {
          method: "PUT",
          credentials: "include",
          body: formdata,
        });
  ...
  return (
    <div
      className={`flex-1 flex flex-col justify-center items-center w-full h-full bg-gradient-to-r from-blue-400 to-purple-400 `}>
      ...
      <div className=" flex flex-col justify-center items-center w-80 m-4 bg-white/20 border border-white/40 rounded-2xl p-8 shadow-xl">
        <h1 className="font-bold text-2xl m-2">UPDATE-POST</h1>
        <form className="text-center" onSubmit={handleUpdate}>
          ...
          <textarea
            ...
            className="border border-gray-300 rounded-lg p-1 text-lg h-23 w-full overflow-y-auto bg-white"></textarea>
          <div className="my-5 w-full">
            <label
              htmlFor="image"
              className="cursor-pointer block w-full text-center py-2 px-4 bg-purple-500 hover:bg-purple-600 text-white rounded-lg transition-colors">
